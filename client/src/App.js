...
const App = () => {
    ...
    const [user, setUser] = useState({ userName: 'user101', passWord: 'user101', cardBalance: 10.5});
    const [admin, setAdmin] = useState({ userName: 'admin', passWord: '123456'});
    //quan sát state của redux store trên console của trình duyệt
    const buyHouseInfo = useSelector((state) => state.user_reducer.house);
    ...
    const houseFilter = useSelector((state) => state.user_reducer.filterHouse);
